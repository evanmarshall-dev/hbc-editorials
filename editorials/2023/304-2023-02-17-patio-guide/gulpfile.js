...
const getLiveURL = (banner) => {
  ...
  switch (banner) {
    case "bay":
      stagingBase =
        `https://www.thebay.com/editorial/${config.project.name}`.toLocaleLowerCase(); //SalesForce
      break;
  }
...
const getPrefix = (banner) => {
  ...
  switch (banner) {
    case "bay":
      prefix =
        "/wcsstore/TheBay/InteractiveEditorials/" +
        config.project.name +
        "/" +
        config.project.pubdate +
        "/";
...
const createPic = (imgDesk = "", alt = "") => {
  ...
  return `
        <picture>
            ...
            <source media="(max-width: 1024px)" srcset="../src/img/${fileName}-mobile.${fileExt}">
...
const createPicCompile = (imgDesk, alt = "", outputType = "default") => {
  ...
  return `
        <picture>
            <source media="(max-width: 1024px)" srcset="${getImageUrl(
              fileName + "." + fileExt,
              outputType,
            )}">
            <source media="(max-width: 1024px)" srcset="${getImageUrl(
              fileName + "-mobile." + fileExt,
              outputType,
            )}">
            <img src="${getImageUrl(
              fileName + "." + fileExt,
              outputType,
            )}" alt="${alt}" style="width:auto;">
        </picture>
    `;
};
  ...
  return (
    gulp
      ...
      .pipe(
        plugin.htmlReplace({
          js: {
            src: config.build_files.js.concat(
              config.app_files.libs,
              config.app_files.js,
            ),
...
const compileScripts = () => {
  return browserify("./src/js/app.js")
    ...
    .pipe(
      plugin.appendPrepend.appendText(
      ...
      plugin.insert.append(function () {
        ...
        for (let i in files) {
          if (getFileExt(files[i]) == "png") {
            returnedValues +=
              getScene7URL() +
              (getFileName(files[i]) + "?scl=1&qlt=76&fmt=webp" + "\n");
          }
        }
        ...
        for (let i in files) {
          if (getFileExt(files[i]) == "png") {
            returnedValues +=
              getScene7URL() +
              (getFileName(files[i]) + "?scl=1&fmt=png" + "\n");
          }
        }
        ...
        for (let i in files) {
          if (getFileExt(files[i]) == "png") {
            returnedValues +=
              getScene7URL() +
              (getFileName(files[i]) + "?scl=1&fmt=png-alpha" + "\n");
          }
        }
...
gulp.task("watch", () => {
  ...
  gulp.watch(
    ["./src/js/*.js", "./src/**/*.js", "./src/js/*.js", "./src/**/**/*.js"],
    buildJs,
  );
...
const compileSplit = gulp.series(
  compileClean,
  compileCode,
  compileCopyImages,
  scene7List,
